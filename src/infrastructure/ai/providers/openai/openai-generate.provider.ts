...
export class OpenAiGenerateProvider implements AiProviderClient {
  ...
  async generate(input: GenerateAiContentInput): Promise<GenerateAiContentResult> {
    ...
    try {
      const response = await this.httpService.axiosRef.post<OpenAiChatCompletionResponse>(
  ...
  private resolveBaseUrl(): string {
    const baseUrl = this.configService.get<string>('aiWorker.openai.baseUrl', '');
  ...
  private resolveApiKey(): string {
    const apiKey = this.configService.get<string>('aiWorker.openai.apiKey', '');
  ...
  private resolveTimeoutMs(): number {
    const timeoutMs = this.configService.get<number>('aiWorker.openai.generateTimeoutMs', 30000);

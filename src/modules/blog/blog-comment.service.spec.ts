...
describe('BlogCommentService', () => {
  ...
  describe('batchUpdateCommentStatus', () => {
    it('ids 为空时应直接返回', async () => {
      await service.batchUpdateCommentStatus({ ids: [], status: BlogCommentStatus.APPROVED });
  ...
  describe('softDeleteComment', () => {
    it('应成功软删除评论', async () => {
      const existing = { id: 1 } as BlogCommentEntity;
      commentRepo.findOne.mockResolvedValue(existing);
      commentRepo.softRemove.mockResolvedValue(existing);

      await service.softDeleteComment(1);

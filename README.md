...
- **本地观察**：通过阅读各业务模块下的 `*.capability.ts` 文件观察 Anchor 声明，结合 `docs/generated/capabilities-current.md` 查看当前能力投影快照。入口模块只是代码导航起点，不声称提供文件级 ownership 视图。（注：`capability:list/docs/docs:check` CLI 工具已在策略一回退 commit `5235d79` 中移除，详见 `plans/Lingeringissue/question26-07-20.md`）
